 ...
 */
public final class LocaleAction extends jswingshell.action.AbstractJssComboAction<Locale> implements LocaleChangeListener {
    ...
    private static final String COMMAND_RUN_CURRENT_LOCALE_MESSAGE_KEY = "midi_player.action.locale.run.current_locale";

    private static final String INTERNAL_COMMAND_BRIEF_HELP = "Sets the shell's locale to ";

    private static final String INTERNAL_COMMAND_BRIEF_HELP_KEY = "midi_player.action.locale.internal_command.help.short";
    ...
    public LocaleAction(Locale[] items, IJssController shellController, String... args) {
        super(items, ACTION_LABEL, ResourceUtils.createImageIcon(ICON_KEY, ACTION_LABEL), shellController, args);
        putValue(Action.LARGE_ICON_KEY, ResourceUtils.createImageIcon(ICON_KEY, ACTION_LABEL, true));
        putValue(Action.ACTION_COMMAND_KEY, getDefaultCommandIdentifier());
        localeChanged();
    }
    ...
    public LocaleAction(ComboBoxModel<Locale> aModel, IJssController shellController, String... args) {
        super(aModel, ACTION_LABEL, ResourceUtils.createImageIcon(ICON_KEY, ACTION_LABEL), shellController, args);
        putValue(Action.LARGE_ICON_KEY, ResourceUtils.createImageIcon(ICON_KEY, ACTION_LABEL, true));
        putValue(Action.ACTION_COMMAND_KEY, getDefaultCommandIdentifier());
        localeChanged();
    }
    ...
    protected boolean doSwitch(IJssController shellController, Locale switchValue) {
        if (switchValue == null) {
            ...
            try {
                ...
                LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_CURRENT_LOCALE_MESSAGE_KEY + "\"", e1);
    ...
    protected Collection<ComboElementAction<Locale>> initInnerElements() {
        ...
        ArrayList<ComboElementAction<Locale>> innerElementActions = new ArrayList<>(dataModel.getSize());

        for (int i = 0, n = dataModel.getSize(); i < n; i++) {
            LocaleAction.LocaleComboElementAction elementAction = this.new LocaleComboElementAction(this, dataModel.getElementAt(i));
