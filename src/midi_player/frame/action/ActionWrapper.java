...
public class ActionWrapper extends AbstractAction {
    ...
    // #########################################################################
    public ActionWrapper(Action wrappedAction, boolean nameAllowed, boolean iconAllowed, boolean largeIconDisplayed) {
    ...
    public void setIconAllowed(boolean iconAllowed) {
        ...
        if (!isIconAllowed()) {
            ...
            firePropertyChange(Action.LARGE_ICON_KEY, wrappedAction.getValue(Action.LARGE_ICON_KEY), null);
        } else {
            firePropertyChange(Action.SMALL_ICON, null, wrappedAction.getValue(Action.SMALL_ICON));
            firePropertyChange(Action.LARGE_ICON_KEY, null, wrappedAction.getValue(Action.LARGE_ICON_KEY));
        }
    }
    ...
    public void putValue(String key, Object value) {
        if (Action.NAME.equals(key) && !isNameAllowed()) {
            return;
        } else if ((Action.SMALL_ICON.equals(key) || Action.LARGE_ICON_KEY.equals(key)) && !isIconAllowed()) {
            return;
        }
