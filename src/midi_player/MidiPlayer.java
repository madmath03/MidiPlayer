 ...
 * 
 * <p>
 * This class encapsulate all the methods necessary to open a MIDI Sequencer,
 * play songs (one at a time) and manage a playlist.
 * </p>
 * 
 * <p>
 * A thread is used to navigate through the playlist when reaching the end of a
 * song or when user requests a change.
 * </p>
 * 
 * @see MidiThread
 *
...
public class MidiPlayer implements MetaEventListener, AutoCloseable {
	...
	/**
	 * Print on the specified print stream a string describing a song's
	 * resource.
	 * 
	 * @param ps
	 *            the print stream
	 ...
	 * 
	 * <p>
	 * A {@code null} value means that no song has been identified as currently
	 * playing.
	 * </p>
	 * 
	 * @see #getCurrentSongIndex()
	...
	// #########################################################################
	/**
	 * Obtains the default MIDI {@code Sequencer}, connected to a default
	 * device.
	 * 
	...
	public final Sequencer getSequencer() {
		if (sequencer == null) {
			...
			try {
				...
				System.out.println("\t" 
						+ sequencer.getDeviceInfo().getDescription());
				System.out.println("\t" 
						+ sequencer.getDeviceInfo().getVendor());
				System.out.println("\t" 
						+ sequencer.getDeviceInfo().getVersion());
				...
				sequencer.open();
				System.out.println(
						"MIDI Sequencer opened and ready for usage.");
	 ...
	 * 
	 * @param songIndex
	 *            new current song index in playlist
	 */
	 ...
	 * Is the MIDI player looping when reaching the end of a song?
	 * 
	 * @return {@code true} if the MIDI player will loop when reaching the end
	 *         of a song
	 */
	...
	/**
	 * Set if the MIDI player should be looping when reaching the end of the
	 * playlist.
	 * 
	 * @param looping
	 *            the playlist looping status
	 ...
	 * 
	 * @param index
	 *            the index in the playlist to move current song position to
	 * 
	 * @return {@code true} if the MIDI player actually started playing
	 */
	 ...
	 * 
	 * @param resource
	 *            the resource in the playlist to move current song position to
	 * 
	 * @return {@code true} if the MIDI player actually started playing
	 */
	...
	/**
	 * Add resources to the playlist and Start playing song for the current
	 * position.
	 * 
	 * @see #startPlaying()
	 ...
	 * 
	 * @param resources
	 *            the resources to add to the playlist
	 * 
	 * @return {@code true} if the MIDI player actually started playing
	 */
	 ...
	 * 
	 * @param force
	 *            force the current song index increase, even if at the edge of
	 *            playlist?
	 * @return {@code true} if the MIDI player current song position changed
	 */
	 ...
	 * 
	 * @return {@code true} if the MIDI player thread is alive and the sequence
	 *         currently playing
	 */
	 ...
	 * 
	 * @return {@code true} if the MIDI player thread is alive and the sequencer
	 *         currently open but <strong>not</strong> playing
	 */
	public final boolean isPaused() {
		...
		return playingThread != null && playingThread.isAlive() 
				&& player.isOpen() && player.getSequence() != null 
				&& !player.isRunning();
	}
	 ...
	 * Is the MIDI player currently playing?
	 * 
	 * @return {@code true} if the MIDI player thread is not alive and the
	 *         sequencer currently not open
	 */
	 ...
	 * 
	 * <p>
	 * The default is 1.0. A value of 1.0 represents the natural rate (the tempo
	 * specified in the sequence), 2.0 means twice as fast, etc.
	 * <p>
	 * Note that the tempo factor cannot be adjusted when external
	 * synchronization is used. In that situation, <code>setTempoFactor</code>
	 ...
	 * 
	 * @return the current song's MIDI resource, {@code null} if no song
	 *         currently playing
	 */
	 ...
	 * 
	 * @param resource
	 *            the given resource to compare with current song's resource
	 * 
	 ...
	 * 
	 * @return the current song's MIDI sequence, {@code null} if no song
	 *         currently playing
	 */
	...
	/**
	 * Print on the standard output a string describing the current song's
	 * resource.
	 */
	 ...
	 * 
	 * @return a string describing the current song's resource, {@code null} if
	 *         no current song
	 */
	...
	// #########################################################################
	/**
	 * Rotates the resources in the playlist between {@code start} and
	 * {@code end} indexes to the specified {@code position} index.
	 * 
	 * @param start
	 *            start index (inclusive) of the sublist of elements in the
	 *            playlist to move
	 * @param end
	 *            end index (inclusive) of the sublist of elements in the
	 *            playlist to move
	 * @param position
	 *            index of the destination for the sublist
	 * @throws IllegalArgumentException
	 *             if the {@code start} is greater than the {@code end} index
	 * @return {@code true} if the elements were moved, {@code false} if the
	 *         {@code position} is between {@code start} and {@code end}
	 */
	public boolean moveSongsTo(final int start, final int end, 
		...
		if (start > end) {
			throw new IllegalArgumentException(
					"Start (" + start 
					+ ") must be lesser or equal to end (" + end 
					+ ") index.");
		}
		...
		List<Object> subList = this.playlist.subList(rotationStart, 
				rotationEnd);
		...
		if (this.currentSongIndex >= rotationStart 
			...
			int offset;
			if (this.currentSongIndex >= start 
					&& this.currentSongIndex <= end) {
				int direction;
				...
				offset = (rotationEnd - rotationStart - Math.abs(distance))
						* direction;
			} else {
				offset = distance;
			}
	...
	/**
	 * Randomly permutes the specified playlist using a default source of
	 * randomness.
	 * 
	 * <p>
	 ...
	 *
	 * @param random
	 *            the source of randomness to use to shuffle the playlist.
	 * @throws UnsupportedOperationException
	 *             if the specified list or its list-iterator does not support
	...
	/**
	 * Sorts the playlist according to the order induced by the specified
	 * comparator.
	 * 
	 * @see #COMPARATOR
	 *
	 ...
	 *
	 * @param resource
	 *            resource to be appended to the playlist
	...
	/**
	 * Appends all of the resources in the specified collection to the end of
	 * this player's resource, in the order that they are returned by the
	 ...
	 *
	 * @param resources
	 *            the resources to add to playlist
	 * @return {@code true} if the playlist changed as a result of the call
	 */
	...
	/**
	 * Inserts all of the resources in the specified collection into this
	 * player's playlist, starting at the specified position.
	 * 
	 * <p>
	 * Shifts the element currently at that position (if any) and any subsequent
	 ...
	 * 
	 * @param index
	 *            the index of the resource to remove from playlist
	 * @return {@code true} if this player's playlist changed as a result of the
	 *         call, {@code false} if no song exists at the given index or if
	 ...
	 * 
	 * @param removedIndex
	 *            the index of the resource removed from playlist
	 * @param removed
	 *            was the resources removed?
	 * @return {@code true} if the post remove actions were done
	 */
	protected boolean afterRemove(final int removedIndex, 
	...
	/**
	 * Removes the first occurrence of the specified resource in this player's
	 * playlist.
	 * 
	 * @param resource
	 *            the resource to remove from playlist
	 * @return {@code true} if this player's playlist changed as a result of the
	 *         call
	 */
	 ...
	 *            the resource removed from playlist
	 * @param removed
	 *            was the resources removed?
	 * @return {@code true} if the post remove actions were done
	 */
	 ...
	 * 
	 * @param indexes
	 *            array of playlist resources' indexes to remove
	 * @return {@code true} if this player's playlist changed as a result of the
	 *         call
	 */
	...
	/**
	 * Removes all of this player's playlist resources that are also contained
	 ...
	 * 
	 * @param resources
	 *            the resources to remove from playlist
	 * @return {@code true} if this player's playlist changed as a result of the
	 *         call
	 */
	 ...
	 * 
	 * @param resources
	 *            the resources removed from playlist
	 * @param removed
	 *            were the resources removed?
	 * @return {@code true} if the post remove actions were done
	 */
	 ...
	 * 
	 * <p>
	 * The thread is responsible for playing a song, waiting until its end or a
	 * request for change, and then move to the next one. Once there is no more
	 * songs to play, the thread stops the MIDI player.
	 * </p>
	 * 
	...
	public class MidiThread extends Thread {
		 ...
		 * 
		 * @param resource
		 *            the resource of the song to play
		 */
		protected final void playSequence(final Object resource) {
			if (resource instanceof Path) {
				...
				MidiPlayer.LOGGER.log(Level.SEVERE, 
						"Impossible to read resource: {0}", resource);
				MidiPlayer.this.reportThrowable(
						"Impossible to read resource: " + resource, null);
			}
		}
		...
		private void playSequence(final File file) {
			try {
				...
				MidiPlayer.this.stopPlaying();
				MidiPlayer.LOGGER.log(Level.SEVERE, 
						"Impossible to read file: " + file, ex);
				MidiPlayer.this.reportThrowable(
						"Impossible to read file: " + file, ex);
			}
		}
		 ...
		 * 
		 * @param is
		 *            the input stream of the MIDI song
		 */
		 ...
		 * 
		 * @param song
		 *            the sequence of the MIDI song
		 * @throws InvalidMidiDataException
		 *             if the sequence contains invalid MIDI data, or is not
		 *             supported.
		 */
				...
				try {
					...
					while (songSequencer != null && songSequencer.isOpen()) {
						...
						this.currentResource = 
								MidiPlayer.this.getCurrentSongResource();
						if (this.currentResource == null) {
							System.out.println(
									"No songs to play. Stopping thread and player...");
							return;
						}
