...
public final class TimeAction extends AbstractJssAction implements LocaleChangeListener {
    ...
    private static final String COMMAND_BRIEF_HELP = "Display the current time and date in the shell.";
    ...
    private static final String COMMAND_HELP_EXAMPLE_KEY = "midi_player.console.action.time.help.long.example";

    private static final String COMMAND_RUN_INVALID_DATE_FORMAT_KEY = "midi_player.console.action.time.run.invalid_date_format";

    private static final String COMMAND_RUN_INVALID_LANGUAGE_FORMAT_KEY = "midi_player.console.action.time.run.invalid_language_format";

    private static final String COMMAND_RUN_INVALID_COUNTRY_FORMAT_KEY = "midi_player.console.action.time.run.invalid_country_format";

    private static final String COMMAND_RUN_INVALID_VARIANT_FORMAT_KEY = "midi_player.console.action.time.run.invalid_variant_format";
    ...
    public static final String getHelp(TimeAction action) {
        if (!commandHelpInitialized && action != null) {
            ...
            try {
                ...
                stringBuilder.append("The date/time format can be specified as well as the language:").append("\n");
                stringBuilder.append("\t").append(commandIdsAsString).append(" [format] [language] [country] [variant] ").append("\n");
                stringBuilder.append("\n");
                stringBuilder.append("If no format and language are provided, the default date format and locale will be used.");
            }
            ...
            if (defaultCommandIdentifier != null) {
                ...
                stringBuilder.append(defaultCommandIdentifier).append(" yyyy-MM-dd'T'HH:mm:ss.SSSZ").append("\n");
                stringBuilder.append("2001-07-04T12:08:56.235-0700").append("\n");

                stringBuilder.append("\n");
                stringBuilder.append(exampleMsg).append("\n");
                stringBuilder.append(defaultCommandIdentifier).append(" \"EEE, MMM d, ''yy\" en us").append("\n");
                stringBuilder.append("Wed, Jul 4, '01").append("\n");

                stringBuilder.append("\n");
                stringBuilder.append(exampleMsg).append("\n");
                stringBuilder.append(defaultCommandIdentifier).append(" \"EEEE dd MMMM yyyy\" th th th").append("\n");
            ...
            switch (args.length) {
                ...
                case 5:
                    try {
                        ...
                        LOGGER.log(Level.WARNING, "Error occurred while parsing date/time format {1} for language {2} and country {3} and variant {4}: {0}", parameters);
                        String msg;
                        try {
                            msg = ResourceUtils.getMessage(COMMAND_RUN_INVALID_VARIANT_FORMAT_KEY, e.getLocalizedMessage(), args[1], args[2], args[3], args[4]);
                        } catch (MissingResourceException e1) {
                            LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_INVALID_VARIANT_FORMAT_KEY + "\"", e1);
                            msg = "Error occurred while parsing date/time format " 
                                    + args[1] 
                                    + " for language " + args[2] 
                                    + " and country " + args[3] 
                                    + " and variant " + args[4] 
                ...
                case 4:
                    try {
                        ...
                        LOGGER.log(Level.WARNING, "Error occurred while parsing date/time format {1} for language {2} and country {3}: {0}", parameters);
                        String msg;
                        try {
                            msg = ResourceUtils.getMessage(COMMAND_RUN_INVALID_COUNTRY_FORMAT_KEY, e.getLocalizedMessage(), args[1], args[2], args[3]);
                        } catch (MissingResourceException e1) {
                            LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_INVALID_COUNTRY_FORMAT_KEY + "\"", e1);
                            msg = "Error occurred while parsing date/time format " 
                                    + args[1] 
                                    + " for language " + args[2] 
                                    + " and country " + args[3] 
                ...
                case 3:
                    try {
                        ...
                        LOGGER.log(Level.WARNING, "Error occurred while parsing date/time format {1} for language {2}: {0}", parameters);
                        String msg;
                        try {
                            msg = ResourceUtils.getMessage(COMMAND_RUN_INVALID_LANGUAGE_FORMAT_KEY, e.getLocalizedMessage(), args[1], args[2]);
                        } catch (MissingResourceException e1) {
                            LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_INVALID_LANGUAGE_FORMAT_KEY + "\"", e1);
                            msg = "Error occurred while parsing date/time format " 
                                    + args[1] 
                                    + " for language " + args[2] 
                                    + ": " + e.getMessage();
                ...
                default:
                    try {
                        ...
                        LOGGER.log(Level.WARNING, "Error occurred while parsing date/time format {1}: {0}", parameters);
                        String msg;
                        try {
                            msg = ResourceUtils.getMessage(COMMAND_RUN_INVALID_DATE_FORMAT_KEY, e.getLocalizedMessage(), args[1]);
                        } catch (MissingResourceException e1) {
                            LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_INVALID_DATE_FORMAT_KEY + "\"", e1);
                            msg = "Error occurred while parsing date/time format " 
                                    + args[1] 
                                    + ": " + e.getMessage();
                        }
