...
public final class WaitAction extends AbstractThreadedJssAction implements LocaleChangeListener {
    ...
    private static final String COMMAND_RUN_INVALID_NUMBER_FORMAT_KEY = "midi_player.console.action.wait.run.invalid_number_format";

    private static final String COMMAND_RUN_NEGATIVE_TIME_KEY = "midi_player.console.action.wait.run.negative_time";

    private static final String COMMAND_RUN_INTERRUPTED_KEY = "midi_player.console.action.wait.run.interrupted";
    ...
    public static final String getHelp(WaitAction action) {
        if (!commandHelpInitialized && action != null) {
            ...
            try {
                ...
                stringBuilder.append("The default time for waiting is 1000 milliseconds (1 second).").append("\n");
                stringBuilder.append("You can define the time for the wait (in milliseconds) as follow:").append("\n");
                stringBuilder.append("\t").append(commandIdsAsString).append(" [time] ").append("\n");
                stringBuilder.append("This action is suited for manual use since it will wait a given time without blocking the current thread.").append("\n");
        ...
        protected Integer doInBackground() throws Exception {
            ...
            try {
                ...
                try {
                    ...
                    LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_INTERRUPTED_KEY + "\"", e1);
                    msg = "Wait action interrupted: " + ex.getMessage();
                }
                this.publish(WaitAction.this.new JssActionWorkerChunk(IJssController.PublicationLevel.WARNING, msg));
