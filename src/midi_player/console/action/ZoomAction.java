 ...
 */
public final class ZoomAction extends jswingshell.action.AbstractJssAction implements LocaleChangeListener {
    ...
    private static final String COMMAND_RUN_INVALID_ARGUMENT_ERROR_KEY = "midi_player.console.action.zoom.run.invalid_argument_error";
        ...
        if (!commandHelpInitialized && action != null) {
            ...
            try {
                stringBuilder.append(ResourceUtils.getMessage(COMMAND_HELP_KEY,
                        commandIdsAsString,
                        zoomInArgument, zoomFitArgument, zoomOutArgument,
                        commandIdentifier, sampleZoomInArgument, sampleZoomOutArgument));
                ...
                stringBuilder.append("\t").append(commandIdsAsString).append(" ").append(zoomInArgument).append("\n");
                stringBuilder.append("\n");
                stringBuilder.append("Shell text size can be reset to its default values:").append("\n");
                stringBuilder.append("\t").append(commandIdsAsString).append(" ").append(zoomFitArgument).append("\n");
                stringBuilder.append("\n");
                stringBuilder.append("Shell text size can be decreased by one point:").append("\n");
                stringBuilder.append("\t").append(commandIdsAsString).append(" ").append(zoomOutArgument).append("\n");
                stringBuilder.append("\n");
                    ...
                    default:
                        try {
                            ...
                            try {
                                ...
                                LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_INVALID_ARGUMENT_ERROR_KEY + "\"", e1);
