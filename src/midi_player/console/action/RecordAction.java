 ...
 *
 * <p>
 * This action's stop and start arguments are used as markers to define the
 * recording scope. When saving the recording to a file, the action will look
 * for the <em>nearest</em> start/stop markers. If none are found, the recording
 * will simply go from the start of the application to the current save, even if
 * save actions were done in the meantime.</p>
 *
 * <p>
 * All successful record actions in the recording scope are omitted.</p>
 *
 * @author Mathieu Brunot
 */
public final class RecordAction extends jswingshell.action.AbstractThreadedJssAction implements LocaleChangeListener {
    ...
    private static final String COMMAND_BRIEF_HELP_KEY = "midi_player.console.action.record.help.short";

    private static final String COMMAND_HELP_KEY = "midi_player.console.action.record.help.long";

    private static final String COMMAND_RUN_FILE_WRITING_ERROR_KEY = "midi_player.console.action.record.run.file_writing_error";

    private static final String COMMAND_RUN_BUFFERED_FILE_CLOSING_ERROR_KEY = "midi_player.console.action.record.run.buffered_file_closing_error";

    private static final String COMMAND_RUN_FILE_CLOSING_ERROR_KEY = "midi_player.console.action.record.run.file_closing_error";
    ...
    public static final String getHelp(RecordAction action) {
        if (!commandHelpInitialized && action != null) {
            ...
            String commandIdsAsString = action.getCommandIdentifiersAsString(),
                    startArgument = RECORD_START,
                    stopArgument = RECORD_STOP,
                    saveArgument = RECORD_SAVE;
            stringBuilder.append(action.getBriefHelp()).append("\n");
            stringBuilder.append("\n");
            try {
                stringBuilder.append(ResourceUtils.getMessage(COMMAND_HELP_KEY, commandIdsAsString, startArgument, stopArgument, saveArgument));
            } catch (MissingResourceException e) {
                LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_HELP_KEY + "\"", e);
                stringBuilder.append("Start recording commands:").append("\n");
                stringBuilder.append("\t").append(commandIdsAsString).append(" ").append(startArgument).append("\n");
                ...
                stringBuilder.append("\t").append(commandIdsAsString).append(" ").append(stopArgument).append("\n");
                ...
                stringBuilder.append("\t").append(commandIdsAsString).append(" ").append(saveArgument).append(" file_path").append("\n");
                ...
                switch (recordAction) {
                    ...
                    case RECORD_SAVE:
                        if (args.length == 3) {
                            ...
                            String[] recordedCommands = extractRecordedCommands((AbstractJssController) shellController);
    ...
    private String[] extractRecordedCommands(AbstractJssController shellController) {
        ...
        for (int i = commandHistory.size() - 1; i >= 0; i--) {
            ...
            if (args != null && args.length > 0) {
                ...
                if ((command.startsWith(IDENTIFIERS[0]) || command.startsWith("\"" + IDENTIFIERS[0])) && args.length > 1) {
    ...
    protected class RecordWorker extends AbstractJssActionWorker {
        ...
        protected Integer doInBackground() throws Exception {
            ...
            try {
                ...
                String msg;
                try {
                    msg = ResourceUtils.getMessage(COMMAND_RUN_FILE_WRITING_ERROR_KEY, e.getLocalizedMessage());
                } catch (MissingResourceException e1) {
                    LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_FILE_WRITING_ERROR_KEY + "\"", e1);
                ...
                if (bw != null) {
                    try {
                        ...
                        String msg;
                        try {
                            msg = ResourceUtils.getMessage(COMMAND_RUN_BUFFERED_FILE_CLOSING_ERROR_KEY, ex.getLocalizedMessage());
                        } catch (MissingResourceException e1) {
                            LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_BUFFERED_FILE_CLOSING_ERROR_KEY + "\"", e1);
                ...
                if (fw != null) {
                    try {
                        ...
                        String msg;
                        try {
                            msg = ResourceUtils.getMessage(COMMAND_RUN_FILE_CLOSING_ERROR_KEY, ex.getLocalizedMessage());
                        } catch (MissingResourceException e1) {
                            LOGGER.log(Level.SEVERE, "Resource not found: \"" + COMMAND_RUN_FILE_CLOSING_ERROR_KEY + "\"", e1);
