...
public class ConsoleFrame extends javax.swing.JFrame implements LocaleChangeListener, AutoCloseable {
    ...
    private void initInternationalization() {
        ...
        ResourceUtils.setTextAndMnemonic(jMenuViewMenuToolbar, "midi_player.console.menu.view.toolbar.text");
    ...
    private void initActions() {
        LocaleAction localeComboAction = (LocaleAction) midiActionFactory.getAction(LocaleAction.DEFAULT_IDENTIFIER);

        // Retrieve actions that hold frame's properties to update
        ToggleToolbarAction toggleToolbarAction = (ToggleToolbarAction) consoleActionFactory.getAction(ToggleToolbarAction.DEFAULT_IDENTIFIER);
        this.displayToolbar = toggleToolbarAction.isSelected();
        ToggleToolbarIconsAction toggleToolbarIconsAction = (ToggleToolbarIconsAction) consoleActionFactory.getAction(ToggleToolbarIconsAction.DEFAULT_IDENTIFIER);
        this.displayToolbarButtonIcons = toggleToolbarIconsAction.isSelected();
        ToggleToolbarNamesAction toggleToolbarNamesAction = (ToggleToolbarNamesAction) consoleActionFactory.getAction(ToggleToolbarNamesAction.DEFAULT_IDENTIFIER);
        this.displayToolbarButtonNames = toggleToolbarNamesAction.isSelected();
        ToggleToolbarLargeIconsAction toggleToolbarLargeIconsAction = (ToggleToolbarLargeIconsAction) consoleActionFactory.getAction(ToggleToolbarLargeIconsAction.DEFAULT_IDENTIFIER);
        this.displayToolbarButtonLargeIcons = toggleToolbarLargeIconsAction.isSelected();

        ToggleToolbarLevelCombo toggleToolbarLevelCombo = (ToggleToolbarLevelCombo) consoleActionFactory.getAction(ToggleToolbarLevelCombo.DEFAULT_IDENTIFIER);
        this.displayToolbarLevelCombo = toggleToolbarLevelCombo.isSelected();
        ToggleToolbarLocaleCombo toggleToolbarLocaleCombo = (ToggleToolbarLocaleCombo) consoleActionFactory.getAction(ToggleToolbarLocaleCombo.DEFAULT_IDENTIFIER);
        this.displayToolbarLocaleCombo = toggleToolbarLocaleCombo.isSelected();

        OpenAction openAction = (OpenAction) consoleActionFactory.getAction(OpenAction.DEFAULT_IDENTIFIER);
        SaveScreenAction saveScreenAction = (SaveScreenAction) consoleActionFactory.getAction(SaveScreenAction.DEFAULT_IDENTIFIER);
        ...
        CopyAction copyAction = (CopyAction) consoleActionFactory.getAction(CopyAction.DEFAULT_IDENTIFIER);
        ...
        PasteAction pasteAction = (PasteAction) consoleActionFactory.getAction(PasteAction.DEFAULT_IDENTIFIER);
        ...
        ClearAction clearAction = (ClearAction) consoleActionFactory.getAction(ClearAction.DEFAULT_IDENTIFIER);

        SelectAllAction selectAllAction = (SelectAllAction) consoleActionFactory.getAction(SelectAllAction.DEFAULT_IDENTIFIER);

        ZoomAction zoomAction = (ZoomAction) consoleActionFactory.getAction(ZoomAction.DEFAULT_IDENTIFIER);
        ...
        FullScreenAction fullScreenAction = (FullScreenAction) consoleActionFactory.getAction(FullScreenAction.DEFAULT_IDENTIFIER);

        RecordAction recordAction = (RecordAction) consoleActionFactory.getAction(RecordAction.DEFAULT_IDENTIFIER);
        ...
        HelpAction helpAction = (HelpAction) consoleActionFactory.getAction(HelpAction.DEFAULT_IDENTIFIER);
        EchoAction echoAction = (EchoAction) consoleActionFactory.getAction(EchoAction.DEFAULT_IDENTIFIER);
        TimeAction timeAction = (TimeAction) consoleActionFactory.getAction(TimeAction.DEFAULT_IDENTIFIER);
        SleepAction sleepAction = (SleepAction) consoleActionFactory.getAction(SleepAction.DEFAULT_IDENTIFIER);
        WaitAction waitAction = (WaitAction) consoleActionFactory.getAction(WaitAction.DEFAULT_IDENTIFIER);

        LevelAction levelComboAction = (LevelAction) consoleActionFactory.getAction(LevelAction.DEFAULT_IDENTIFIER);
     ...
     * @see
     * http://www.programcreek.com/java-api-examples/index.php?source_dir=jreepad-master/src/jreepad/editor/TextTransferHandler.java
     */
    static class TextAreaFilesTransferHandler extends TransferHandler {

        private static final Logger LOGGER = Logger.getLogger(TextAreaFilesTransferHandler.class.getName());
         ...
         * <p>
         * We need this information when performing a MOVE in order to remove
         * the dragged text from the source.</p>
         */
        ...
        public boolean canImport(TransferHandler.TransferSupport info) {
            boolean b = (info.getComponent() == textArea || info.getComponent() == container)
                    && (info.isDataFlavorSupported(filesFlavor) || info.isDataFlavorSupported(DataFlavor.stringFlavor));
        ...
        public boolean importData(TransferHandler.TransferSupport info) {
            ...
            if (info.isDataFlavorSupported(filesFlavor)) {
                ...
                if (files != null) {
                    ...
                    for (File file : files) {
                        stringBuilder
                                .append(controller.getCommandParameterSeparator())
